...
if (!mysqli_query($koneksi, $delete_ortu_query)){
    echo"Error: " . $delete_ortu_query . "<br>" . mysqli_errno($koneksi);
}
$delete_wali_query = "DELETE FROM data_ortu WHERE id=(SELECT id FROM data_siswa WHERE id='$id')";
if (!mysqli_query($koneksi, $delete_wali_query)){
    echo"Error: " . $delete_wali_query . "<br>" . mysqli_errno($koneksi);
}
